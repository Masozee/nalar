...
export default function Publications() {
  ...
  const [apiPublications, setApiPublications] = useState<ApiPublication[]>([]);
  ...
  useEffect(() => {
    const fetchPublications = async () => {
      ...
      try {
        ...
        setApiPublications(data.results || []);
        ...
        const convertedPublications: Publication[] = (data.results || []).map((pub: ApiPublication) => ({
          ...
          excerpt: pub.description?.substring(0, 200) + '...' || '',
          image: pub.image,
          topics: pub.topic?.map(t => t.name) || [],
          featured: false, // Set featured based on your criteria
          download: pub.file || '',
        }));
        
        if (convertedPublications.length > 0) {
          // Set a featured publication (first one or based on criteria)
          convertedPublications[0].featured = true;
          ...
          setFeaturedPublication(PUBLICATIONS.find(pub => pub.featured) || PUBLICATIONS[0]);
        }
      } catch (err) {
  ...
  }, [searchQuery, activeFilters, publications]);
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
              <div className="animate-pulse space-y-8">
                ...
                <div className="md:flex gap-8">
                  <div className="md:w-2/5 h-64 bg-gray-200 rounded"></div>
                      ...
                      >
                        ...
                        <div className="p-6">
                          <h3 className="text-lg font-bold text-primary mb-2 line-clamp-2">
                            <Link href={`/publication/${publication.slug}`} className="hover:underline">
                              {publication.title}
                            </Link>
                          </h3>
