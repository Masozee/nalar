...
export default function ExpenseRequestDetailPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      setProcessing(true)
      await expenseRequestApi.submit(expense.id)
  ...
  const handleCancel = async () => {
    ...
    try {
      setProcessing(true)
      await expenseRequestApi.cancel(expense.id)
      setShowCancelDialog(false)
      fetchExpense()
    } catch (error) {
      console.error("Error cancelling expense:", error)
      alert("Failed to cancel expense request")
    } finally {
      setProcessing(false)
    }
  }
  ...
  const canSubmit = expense.status === 'draft' && expense.items.length > 0
  const canEdit = expense.status === 'draft'
  const canCancel = !['paid', 'processing', 'cancelled'].includes(expense.status)
  ...
  return (
    <SidebarProvider style={{ "--sidebar-width": "350px" } as React.CSSProperties}>
          ...
          <div className="flex items-center gap-2">
            ...
            <Breadcrumb>
              <BreadcrumbList>
                ...
                <BreadcrumbSeparator />
                <BreadcrumbItem>
                  <BreadcrumbLink href="/finance/my-requests">My Requests</BreadcrumbLink>
                </BreadcrumbItem>
                <BreadcrumbSeparator />
                <BreadcrumbItem>
            ...
            <div className="flex gap-2">
              ...
              {canEdit && (
                <Button variant="outline" asChild>
                  <Link href={`/finance/my-requests/${expense.id}/edit`}>
      ...
      <Dialog open={showSubmitDialog} onOpenChange={setShowSubmitDialog}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Are you sure you want to submit this expense request for approval?
              Once submitted, you won't be able to edit it.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button variant="outline" onClick={() => setShowSubmitDialog(false)} disabled={processing}>
              Cancel
            </Button>
      ...
      <Dialog open={showCancelDialog} onOpenChange={setShowCancelDialog}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Cancel Expense Request</DialogTitle>
            <DialogDescription>
